...
class FileDownloadList {
	...
	private FileDownloadList() {
		list = new ArrayList<BaseDownloadTask>();
	}
	...
	List<BaseDownloadTask> copy(final FileDownloadListener listener) {
		final List<BaseDownloadTask> targetList = new ArrayList<BaseDownloadTask>();
		synchronized (list) {
	 ...
	 *            will be remove
	 * @param removeByStatus
	 *            must remove by status
	 *            {@link com.liulishuo.filedownloader.model.FileDownloadStatus#warn}
	 ...
	 */
	boolean remove(final BaseDownloadTask willRemoveDownload,
			final byte removeByStatus) {
		boolean succeed;
		synchronized (list) {
			succeed = list.remove(willRemoveDownload);
		}
		FileDownloadLog.v(this, "remove %s left %d %d", willRemoveDownload,
				removeByStatus, list.size());
		...
		if (succeed) {
			...
			FileDownloadLog.e(this, "remove error, not exist: %s",
					willRemoveDownload);
		}
